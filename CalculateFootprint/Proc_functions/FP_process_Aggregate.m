...
COMP(i).CH4tile=zeros(length(FX),length(FY));% Flux map footprint-weighed
...
COMP(i).CH4tile_abs=nan(length(FX),length(FY));% Flux map for CH4 absolute values
...
COMP(i).footCountCH4=zeros(length(FX),length(FY));% number of time a cell within the end countourMark is counted for CH4 footprints
...
COMP(i).footWeightCH4=zeros(length(FX),length(FY));
end
...
if ~isnan(ustar(i)*Lo(i)*sv(i)*zo(i)*z(i)*d(i)*windir(i)*h(i)*ubar(i))
    ...
    for j=1:3
        ...
        tmpA = cat(3,COMP(j).CH4tile,CH4_gen); COMP(j).CH4tile = nansum(tmpA,3);
        ...
        tmpA = cat(3,COMP(j).CH4tile_abs,CH4_genL); COMP(j).CH4tile_abs = nansum(tmpA,3);
        ...
        if ~isnan(FCH4(i))
        tmpW = cat(3,COMP(j).footWeightCH4,(FPall(j).fCrop)); COMP(j).footWeightCH4 = nansum(tmpW,3);    
        tmpS = cat(3,COMP(j).footCountCH4,~isnan(FPall(j).fCrop)); COMP(j).footCountCH4 = nansum(tmpS,3);
        end
